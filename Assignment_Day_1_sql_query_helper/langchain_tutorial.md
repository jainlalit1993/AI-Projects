...
A step-by-step guide to understanding LangChain by building a SQL Query Helper agent.
...
8. [Putting It All Together -- SQL Query Helper](#8-putting-it-all-together----sql-query-helper)
...
def generate_sql_query(request: str) -> str:
    """Creates a clean SQL SELECT query from a plain English data request.
    Use this tool FIRST when the user describes data they want.
    Input should be the user's data request in plain English.
    Returns a formatted SQL query using standard table names like customers, orders, products."""
```
...
```python
SYSTEM_PROMPT = """You are a SQL Query Helper assistant.
When the user describes data they want in plain English:
1. First, use the generate_sql_query tool to create a SQL query.
2. Then, use the explain_sql_query tool to explain it line by line.
3. Return the SQL query and explanation."""
```

---

## 8. Putting It All Together -- SQL Query Helper

Here's the full flow of our SQL Query Helper project:

```
User: "show me all customers who placed an order in the last 30 days"
  |
  v
Agent receives the message
  |
  v
Agent THINKS: "I should generate a SQL query first"
  |
  v
Agent CALLS: generate_sql_query("show me all customers who placed an order in the last 30 days")
  |
  v
generate_sql_query tool:
  - Creates a PromptTemplate for SQL generation
  - Fills in the user's request
  - Sends to LLM
  - Returns a clean SQL SELECT query
  |
  v
Agent OBSERVES the SQL query
  |
  v
Agent THINKS: "Now I should explain this query"
  |
  v
Agent CALLS: explain_sql_query(the_sql_query)
  |
  v
explain_sql_query tool:
  - Creates a PromptTemplate for explanation
  - Fills in the SQL query
  - Sends to LLM
  - Returns line-by-line explanation in plain English
  |
  v
Agent OBSERVES the explanation
  |
  v
Agent THINKS: "This looks complete, I'm done"
  |
  v
Agent RESPONDS with the SQL query and explanation
```
...
When you run `python sql_query_helper_agent.py`, here's what happens in order:
...
| 4 | Two tools are defined | `@tool def generate_sql_query(...)` and `@tool def explain_sql_query(...)` |
| 5 | Agent is created with LLM + tools + prompt | `create_agent(model=llm, tools=tools, ...)` |
| 6 | Interactive loop starts | `input("Your data request: ")` |
| 7 | User enters a request | e.g., "show me all customers who placed an order in the last 30 days" |
| 8 | Agent runs the tool-calling loop | `agent_graph.invoke({"messages": [...]})` |
| 9 | Final SQL and explanation is printed | `print(final_output)` |
...
1. **Add more tools** -- e.g., a `validate_sql` tool to check syntax or a `optimize_sql` tool for performance
2. **Try different models** -- swap `gpt-4.1-mini` for `gpt-4o` and compare quality
3. **Add memory** -- let the agent remember previous queries in the conversation
